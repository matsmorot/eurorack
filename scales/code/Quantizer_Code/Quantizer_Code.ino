...
// 12 March 2022
// Adaptation for "Scales" version by M.Almen, 2025-04-22
...
#define biMonoPin 16 // uni-polar: 0V/+5V, bi-polar: -3.5V/+5V
#define trigPin 8
...
bool biCV = false;  // startup value of incoming CV range. uni-polar: 0V/+5V, bi-polar: -3.5V/+5V
byte shMode = true; // Sample & Hold mode
byte qntz;          // variable for storing quantized value;
byte note;
byte offset = 0; // transpose offset
byte glideIndex = 0;
byte btnState = 0; // binary flags for storing button states [UP, DOWN, SCALE, TRANSPOSE, GLIDE]
...
MyAnalog adc(A3, 10, octaves * 12);                             // pin, error margin, max value (5*12 semitones).
...
byte ledNoteMap[12] = {11, 8, 5, 10, 0, 3, 2, 9, 4, 7, 1, 6};  // remaps the indexes for the Charlie Plexing LEDS
byte ledLogicMap[12] = {11, 5, 0, 3, 9, 7, 6, 8, 10, 2, 4, 1}; // remaps the indexes for the Charlie Plexing LEDS
...
byte btnPins[4] = {18, 19, 12, 11};         //UP, DOWN , SCALE, TRANSPOSE
...
void cycleLeds() {
  byte totalPins = sizeof(ledLogicMap) / sizeof(ledLogicMap[0]);
  
  for (byte i = 0; i < totalPins; i++) {
    myLeds.printLed(ledLogicMap[i]);
    Serial.print(ledLogicMap[i]);
    delay(1000);
  }
